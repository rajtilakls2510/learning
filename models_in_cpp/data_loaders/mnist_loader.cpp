...
Loader::Loader(std::string data_path, int batch_size) : gen(std::random_device{}()),
...
std::pair<torch::Tensor, torch::Tensor> Loader::read_images_labels(std::string &images_path, std::string &labels_path)
{
...
cv::Mat Loader::tensor_to_cv_mat(torch::Tensor image_tensor)
{
    ...
    else if (image_tensor.sizes().size() == 2)
    { // Already [H, W]
        return cv::Mat(image_tensor.size(0), image_tensor.size(1), CV_8UC1, image_tensor.data_ptr());
    }
    else
