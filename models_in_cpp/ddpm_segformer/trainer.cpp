...
Trainer::Trainer(
    ...
    model = segformer::SegFormerMnist(
            /*img_size*/ 28,
            /*in_chans*/ 1,
            /*embed_dims*/ std::vector<int>({256, 512}),
            /*num_heads*/ std::vector<int>({2, 4}),
            /*mlp_ratios*/ std::vector<float>({4.0, 4.0}),
            /*qkv_bias*/ true,
            /*drop_rate*/ 0.0,
            /*drop_path_rate*/ 0.0,
            /*depths*/ std::vector<int>({3, 3}),
            /*sr_ratios*/ std::vector<int>({2, 1}),
            /*decoder_embed_dim*/ 256,
            /*out_channels*/ 1);
    optimizer = std::make_shared<torch::optim::Adam>(
            model->parameters(), torch::optim::AdamOptions(5e-4));
