...
SimpleUNetClassifierImpl::SimpleUNetClassifierImpl(
        int img_size,
        int in_channels,
        int num_classes,
        int time_dim,
    ...
    int last_dim = channel_sequence[(int)channel_sequence.size() - 1];
    out = Sequential();
    out->push_back(Linear(LinearOptions(last_dim, last_dim * 4)));
    out->push_back(ReLU());
    out->push_back(Linear(LinearOptions(last_dim * 4, num_classes)));
    register_module("out", out);
}

Tensor SimpleUNetClassifierImpl::forward(Tensor x, Tensor t) {
    // x : [B, in_channels, H, W], t : [B,]

    torch::Tensor h = stem(x);

    for (int i = 0; i < down_blocks.size(); i++) h = down_blocks[i](h, t);

    h = h.mean({2, 3});
    return out->forward(h);  // [B, num_classes]
}
