...
int main(int argc, char* argv[]) {
    ...
    float guidance_scale = 10.0;             // how strongly to guide (try 0–10)
    ...
    auto classifier = unet::SimpleUNetClassifier(
            /*img_size*/ 28,
            /*in_channels*/ 1,
            /*num_classes*/ 10,
            /*time_dim*/ 256,
    ...
    torch::load(classifier, checkpoint_path + "/classifier.pth");
    model->to(device);
    model->eval();
    ...
    for (int t = max_diffusion_time; t > 0; t--) {
        ...
        torch::Tensor noise_predicted;
        {
            torch::NoGradGuard no_grad;
            auto outputs = model->forward(x, timesteps);
            noise_predicted = outputs.index({Slice(), Slice(0, 1)});  // outputs[:, 0:1]
        }
        x.set_requires_grad(true);
        torch::Tensor logits = classifier->forward(x, timesteps);
        torch::Tensor log_probs = torch::log_softmax(logits, 1);
        torch::Tensor selected = log_probs.index({Slice(), target_class}).mean();
        torch::Tensor grad = torch::autograd::grad({selected}, {x}, {}, true, true)[0];
        ...
        x += guidance_scale * torch::exp(log_var) * grad;
