...
void Trainer::train_step(Batch batch, double* model_loss) {
    model->train();
    ...
    int B = labels.size(0);
    // With probability p_drop, set class = -1 (unconditional)
    double p_drop = 0.1;  // 10% Dropout like original classifier-free guidance
    auto rand_vals = torch::rand({B}, torch::TensorOptions().device(device));
    auto uncond_mask = (rand_vals < p_drop);  // boolean mask
    // Create class_ids tensor
    auto class_ids = labels.clone();
    class_ids.index_put_({uncond_mask}, -1);
    ...
    auto outputs = model->forward(x_noisy, t, class_ids);
    ...
    auto loss_tensor = model_loss_tensor;
...
void Trainer::test_step(Batch batch, double* model_loss) {
    model->eval();
    auto images = batch.images.to(device);
    images = 2 * images - 1;  // Scale between [-1,1]
    auto labels = batch.labels.to(device);
...
void Trainer::learn(int epochs, int batch_size) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        // Train
        int n_batch = 0;
        double avg_train_model_loss = 0;
        while (true) {
            try {
                Batch b = loader.get_train_batch();
                double model_loss = 0;
                train_step(b, &model_loss);
                n_batch++;
        ...
        // Test
        n_batch = 0;
        double avg_test_model_loss = 0;
        while (true) {
            try {
                Batch b = loader.get_test_batch();
                double model_loss = 0;
                test_step(b, &model_loss);
                n_batch++;
                avg_test_model_loss =
