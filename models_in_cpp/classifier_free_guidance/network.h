...
class ClassEmbeddingImpl : public Module {
public:
    ClassEmbeddingImpl(int num_classes, int emb_dim, int out_dim);
    torch::Tensor forward(torch::Tensor class_ids);

private:
    int num_classes, emb_dim, out_dim;
    Embedding emb{nullptr};
    Linear proj{nullptr};
};

TORCH_MODULE(ClassEmbedding);

class UNetDownsampleImpl : public Module {
public:
    UNetDownsampleImpl(int in_ch, int out_ch, int time_dim, int kernel_size, int num_classes);
    torch::Tensor forward(torch::Tensor x, torch::Tensor t, torch::Tensor class_ids);
...
private:
    ...
    ClassEmbedding class_proj{nullptr};
};
...
public:
    UNetUpsampleImpl(int in_ch, int out_ch, int time_dim, int kernel_size, int num_classes);
    torch::Tensor forward(torch::Tensor x, torch::Tensor t, torch::Tensor class_ids);
