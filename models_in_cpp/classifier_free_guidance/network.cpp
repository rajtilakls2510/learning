...
ClassEmbeddingImpl::ClassEmbeddingImpl(int num_classes, int emb_dim, int out_dim)
    : num_classes(num_classes), emb_dim(emb_dim), out_dim(out_dim) {
    // +1 so index 0 = "unconditional token"
    emb = register_module("emb", torch::nn::Embedding(num_classes + 1, emb_dim));
    proj = register_module("proj", torch::nn::Linear(emb_dim, out_dim));
}

torch::Tensor ClassEmbeddingImpl::forward(torch::Tensor class_ids) {
    // class_ids: [B] with values in [-1, num_classes-1]
    // Shift to [0, num_classes], where 0 = unconditional
    auto shifted = class_ids + 1;
    auto e = emb(shifted);            // [B, emb_dim]
    auto out = torch::relu(proj(e));  // [B, out_dim]
    return out;
}

UNetDownsampleImpl::UNetDownsampleImpl(
        int in_ch, int out_ch, int time_dim, int kernel_size, int num_classes)
    ...
    class_proj = register_module("class_proj", ClassEmbedding(num_classes, time_dim, out_ch));
}

torch::Tensor UNetDownsampleImpl::forward(
        torch::Tensor x, torch::Tensor t, torch::Tensor class_ids) {
    // x : [B, C, H, W] t : [B,] class_ids: [B,]
    x = bn1(functional::relu(conv_in(x)));
    torch::Tensor t_emb = functional::relu(time_proj(time_encoder(t)));
    t_emb = t_emb + class_proj(class_ids);
    x += t_emb.unsqueeze(-1).unsqueeze(-1);
    x = bn2(functional::relu(conv_feat(x)));
    return spatial(x);
}

UNetUpsampleImpl::UNetUpsampleImpl(
        int in_ch, int out_ch, int time_dim, int kernel_size, int num_classes)
    : in_ch(in_ch), out_ch(out_ch), time_dim(time_dim), kernel_size(kernel_size) {
    time_encoder = register_module("time_encoder", TimePosEncoding(time_dim));
    conv_in = register_module(
...
torch::Tensor UNetUpsampleImpl::forward(torch::Tensor x, torch::Tensor t, torch::Tensor class_ids) {
    // x : [B, C, H, W] t : [B,]
    x = bn1(functional::relu(conv_in(x)));
    torch::Tensor t_emb = functional::relu(time_proj(time_encoder(t)));
    t_emb = t_emb + class_proj(class_ids);
...
torch::Tensor SimpleUNetImpl::forward(torch::Tensor x, torch::Tensor t, torch::Tensor class_ids) {
    ...
    for (int i = 0; i < down_blocks.size(); i++) {
        h = down_blocks[i](h, t, class_ids);
        skips.push_back(h);
    }

    for (int i = 0; i < up_blocks.size(); i++) {
        h = up_blocks[i](
                torch::cat({h, skips[(int)up_blocks.size() - i - 1]}, /*dim*/ 1), t, class_ids);
    }
